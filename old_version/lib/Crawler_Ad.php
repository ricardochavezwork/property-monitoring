...
class Crawler_Ad extends DataClass {

  const TABLE_NAME = "AnnunciAcquisizioni";
  ...
  public function Save(){
    ...
    $query = sprintf("CALL SaveAnnunciAcquisizioni(%d, %s, %s, %s, %s, %d, %d, %d, %s, %s, %d, %d, %d, %s, %s, %d, %s, %d);",
      $this->Id,
  ...
  public static function SearchByLink($string){
    ...
    if($string){
      $sql = sprintf("SELECT COUNT(*) as Total, Id FROM AnnunciAcquisizioni WHERE Link = '%s'", $Database->Escape($string));
      ...
      /*if($exists){
        $update_sql = sprintf("UPDATE AnnunciAcquisizioni SET TettoMassimo = %d, DataAggiornamento = CURRENT_TIMESTAMP WHERE Id = '%d';", $tettoMassimo, $found_id);
