...
class Crawler_Ad_Rent extends DataClass {

  const TABLE_NAME = "AnnunciAffitto";
  ...
  public function Save(){
    ...
    $query = sprintf("CALL SaveAnnunciAffitto(%d, %s, %s, %s, %s, %d, %d, %d, %s, %s, %d, %d, %d, %s, %s, %d, %s, %d);",
      $this->Id,
  ...
  public static function SearchByLink($string){
    ...
    if($string){
      $sql = sprintf("SELECT COUNT(*) as Total, Id FROM AnnunciAffitto WHERE Link = '%s'", $Database->Escape($string));
      ...
      /*if($exists){
        $update_sql = sprintf("UPDATE AnnunciAffitto SET TettoMassimo = %d, DataAggiornamento = CURRENT_TIMESTAMP WHERE Id = '%d';", $tettoMassimo, $found_id);
