...
class ZohoCrmApi  {
  ...
  public static function LinkAdExists($link = "", $module = ""){
    try {
      ...
      if(!$link)
        throw new Exception("ZohoCrmApi:LinkAdExists -> Link variable required", 1);

      if(!$module)
        throw new Exception("ZohoCrmApi:LinkAdExists -> Module variable required", 1);
  ...
  public static function fillRecordFromImmobiliare($record, $immobiliare, $existingRecordId){
    try {
      ...
      if(!$immobiliare)
        throw new Exception("ZohoCrmApi:fillRecordFromImmobiliare -> Immobiliare variable required", 1);
  ...
  public static function upsertRecords($records, $moduleName){
    try {
      if(!$records)
        throw new Exception("ZohoCrmApi:upsertRecords -> Records variable required", 1);

      if(!$moduleName)
        throw new Exception("ZohoCrmApi:upsertRecords -> ModuleName variable required", 1);
